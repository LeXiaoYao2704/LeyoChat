    ...
    : QMainWindow{parent}, d_ptr(new ElaWindowPrivate())
{
    ...
    d->_movieThrottleTimer = new QTimer(this);
    d->_movieThrottleTimer->setInterval(66); // ~15fps 节流
    d->_movieThrottleTimer->setSingleShot(true);
    connect(d->_movieThrottleTimer, &QTimer::timeout, this, [=]() {
        // 预缩放当前帧并缓存
        QPixmap raw = d->_windowPaintMovie->currentPixmap();
        if (!raw.isNull() && !size().isEmpty()) {
            d->_scaledMovieFrame = raw.scaled(size(), Qt::KeepAspectRatioByExpanding,
                                              Qt::FastTransformation);
            d->_scaledMovieFrameWindowSize = size();
        }
        update();
    });
    connect(d->_windowPaintMovie, &QMovie::frameChanged, this, [=]() {
        if (!d->_movieThrottleTimer->isActive()) {
            d->_movieThrottleTimer->start();
        }
    });
}
...
void ElaWindow::paintEvent(QPaintEvent* event)
{
    ...
    if (d->_pWindowPaintMode != ElaWindowType::Movie)
    ...
    case ElaApplicationType::Normal:
    {
        ...
        {
            if (!d->_scaledMovieFrame.isNull()) {
                // 使用预缩放缓存帧，窗口尺寸变化时重新缩放
                if (d->_scaledMovieFrameWindowSize != size()) {
                    QPixmap raw = d->_windowPaintMovie->currentPixmap();
                    if (!raw.isNull()) {
                        d->_scaledMovieFrame = raw.scaled(size(), Qt::KeepAspectRatioByExpanding,
                                                          Qt::FastTransformation);
                ...
                // 居中裁剪绘制
                int sx = (d->_scaledMovieFrame.width() - rect().width()) / 2;
                int sy = (d->_scaledMovieFrame.height() - rect().height()) / 2;
                painter.drawPixmap(rect(), d->_scaledMovieFrame,
                    QRect(qMax(0, sx), qMax(0, sy), rect().width(), rect().height()));
            }
            break;
        }
        default:
